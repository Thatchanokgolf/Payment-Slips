...
export async function appendPaymentRow({ fields, categoryName, image }) {
  ...
  const linkCell = uploaded ? `=HYPERLINK("${uploaded.viewUrl}","View slip")` : "";
  const imageCell = uploaded ? `=IMAGE("${uploaded.imageUrl}")` : "";

  const sheets = google.sheets({ version: "v4", auth });
  await sheets.spreadsheets.values.append({
    spreadsheetId: SHEET_ID,
    range: cell("A1"),
    valueInputOption: "USER_ENTERED", // so =HYPERLINK/=IMAGE are parsed as formulas
    insertDataOption: "INSERT_ROWS",
    requestBody: {
      values: [[
        fields.when.toISOString(),
        fields.receiver,
        fields.amt,
        fields.cat,
        categoryName,
